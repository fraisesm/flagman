<template>
  <div class="app-shell">
    <aside class="sidebar">
      ...
      <div class="backend-box" v-if="token">
        <label>Вы вошли</label>
        <div class="quick-ids">
          <span>{{ currentUser?.full_name ?? '—' }}</span>
          <span>{{ currentUser?.email ?? '—' }}</span>
        </div>
        <button class="mini-button" style="margin-top:6px" @click="logout">Выйти</button>
      </div>
      ...
      <div class="sidebar__footer">
        <p>Все действия идут в реальные backend endpoints через fetch.</p>
      </div>
    </aside>
    ...
    <main class="main">
      ...
      <!-- AUTH TAB -->
      <section v-if="activeTab === 'auth'" class="content-grid">
        <article class="panel">
        ...
        <article class="panel">
          ...
          <div class="form">
            <input v-model="loginForm.email" class="input" placeholder="Email" />
            <input v-model="loginForm.password" class="input" type="password" placeholder="Пароль" />
            <button class="button" @click="login">Войти</button>
          </div>
          <div v-if="token" class="list-item" style="margin-top:12px">
            <strong>Токен получен ✓</strong>
            <span style="word-break:break-all;font-size:11px">{{ token.slice(0, 60) }}...</span>
          </div>
        </article>
      </section>
      ...
      <section v-if="activeTab === 'structure'" class="content-grid">
        ...
        <article class="panel">
          ...
          <div class="form">
            ...
            <div class="checks">
              <label class="checkbox"><input v-model="roleForm.can_send_document" type="checkbox" /><span>can_send_document</span></label>
              <label class="checkbox"><input v-model="roleForm.can_sign_document" type="checkbox" /><span>can_sign_document</span></label>
              <label class="checkbox"><input v-model="roleForm.can_manage_department" type="checkbox" /><span>can_manage_department</span></label>
            </div>
      ...
      <section v-if="activeTab === 'documents'" class="content-grid">
        ...
        <article class="panel">
          ...
          <div class="inline-actions">
            <button class="mini-button" @click="signForm.document_id = lastIds.document_id">Подставить doc_id</button>
          </div>
          <div class="form">
      ...
      <section v-if="activeTab === 'lists'" class="content-grid">
        <article class="panel">
          <div class="panel__head"><h3>Inbox</h3><span class="tag">/documents/inbox</span></div>
          <div class="inline-actions">
        ...
        <article class="panel">
          <div class="panel__head"><h3>Outbox</h3><span class="tag">/documents/outbox</span></div>
          <div class="inline-actions">
        ...
        <article class="panel">
          <div class="panel__head"><h3>Pending</h3><span class="tag">/documents/pending</span></div>
          <div class="inline-actions">
        ...
        <article class="panel">
          <div class="panel__head"><h3>Статус документа</h3><span class="tag">/documents/status/:doc/:user</span></div>
          <div class="inline-actions">
          ...
          <div class="json-box"><pre>{{ pretty(statusResult) }}</pre></div>
        </article>
      </section>
...
const currentTabTitle = computed(() => tabs.find((i) => i.key === activeTab.value)?.label || '')
...
function logout() {
  ...
  output.value = { detail: 'Вы вышли из системы' }
}
...
function normalizeError(data) {
  ...
  if (Array.isArray(data.detail))
    return data.detail.map((i) => `${(i.loc || []).join(' -> ')}: ${i.msg}`).join('\n')
...
function updateLastIds(data) {
  ...
  if ('id' in data && 'name' in data && !('title' in data) && !('email' in data)) {
    if (organizationForm.name === data.name) lastIds.organization_id = data.id
  }
...
// ---- fill helpers ----
function fillOrganizationName() { organizationForm.name = 'ООО Флагман Demo' }
...
function setSenderRole() {
  ...
  roleForm.can_send_document = true; roleForm.can_sign_document = false; roleForm.can_manage_department = false
}
function setSignerRole() {
  roleForm.role_name = 'signer'
  roleForm.can_send_document = false; roleForm.can_sign_document = true; roleForm.can_manage_department = false
}
...
async function assignRole() {
  try { await apiRequest('/access/assign-role', 'POST', roleForm) }
  catch (e) { output.value = { error: e.message } }
}
...
async function signDocument() {
  try { await apiRequest('/signatures/sign', 'POST', signForm) }
  catch (e) { output.value = { error: e.message } }
}

// ---- List actions ----
async function loadInbox() {
  try { inboxList.value = await apiRequest('/documents/inbox', 'POST', { user_id: inboxUserId.value }) }
  catch (e) { output.value = { error: e.message } }
}
async function loadOutbox() {
  try { outboxList.value = await apiRequest('/documents/outbox', 'POST', { user_id: outboxUserId.value }) }
  catch (e) { output.value = { error: e.message } }
}
async function loadPending() {
  try { pendingList.value = await apiRequest('/documents/pending', 'POST', { user_id: pendingUserId.value }) }
  catch (e) { output.value = { error: e.message } }
}
async function loadStatus() {
  try {
    statusResult.value = await apiRequest(
      `/documents/status/${statusForm.document_id}/${statusForm.recipient_user_id}`, 'GET'
    )
  } catch (e) { output.value = { error: e.message } }
}
</script>
