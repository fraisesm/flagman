            ...
            <div v-if="isBoss" class="section-block">
              ...
              <div class="cards-grid">
                <div class="card">
                  ...
                  <p v-if="lastIds.document_id" class="hint success">✓ Документ создан, ID: <code>{{ lastIds.document_id }}</code></p>
                </div>
                <div class="card">
                  <div class="card__title">📨 Отправить получателю</div>
                  <div class="form-col">
...
async function sendDocument() {
  try { await apiRequest('/documents/send', 'POST', sendForm); showToast('Документ отправлен на подпись!') }
  catch(e) { showToast(e.message, 'error') }
}
